...
public class OrderServiceImpl implements OrderService {
    ...
    public Order createOrder(long userId, long certificateId) {
        User user = userDao.findById(userId).orElseThrow(() -> new ElementSearchException(ServiceMessageManager.ELEMENT_SEARCH_KEY.getMessage(userId)));
        GiftCertificate certificate = giftCertificateDao.findById(certificateId).orElseThrow(() -> new ElementSearchException(
                ServiceMessageManager.ELEMENT_SEARCH_KEY.getMessage(certificateId))
        );
        ...
        if(user.getCash().compareTo(certificate.getPrice()) >= 0) {
            ...
            throw new OperationNotPerformedException(ServiceMessageManager.OPERATION_NOT_PERFORMED.getMessage(userId, certificateId));
        }
        return order;
    }
    ...
    public Order findById(long id) {
        ...
        return orderOptional.orElseThrow(() -> new ElementSearchException(ServiceMessageManager.ELEMENT_SEARCH_KEY.getMessage(id)));
    }
